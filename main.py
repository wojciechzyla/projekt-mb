...
def number_of_orders_according_to_date_plot():
    print("Number of orders depending on the date")
    ocel_ = ocel
    ocel_.get_extended_table().head()
    df = ocel_.get_extended_table()
...
#================= FOR IPYNB ================
display_basic_info = interact(show_basic_information, log_path = "logs/running-example.jsonocel")
relations_eid = interact(show_relations_for_eid_and_object_type, eid="1.0", type=pm4py.ocel_get_object_types(ocel))
